...
module mini_itx(h=18) {
    translate([6.35,33.02,0]) abstandshalter_atx(h=h); // 1
    translate([6.35,165.1,0]) abstandshalter_atx(h=h); // 2
    translate([6.35+157,165.1,0]) abstandshalter_atx(h=h); // 3
    translate([6.35+157,10.16,0]) abstandshalter_atx(h=h); // 4
}

// Schraubsockel (einzeln) für ATX Mainboards
module abstandshalter_atx(h=13) {
    difference() {
        translate([0,0,h/2]) cylinder(d=6, h=h, center=true, $fn=50);
...
module cut_spax2_deep(elevation=5) {
    translate([0,0,elevation])union() {
        ...
        translate([0,0,-0.75+0.01]) cylinder(d1=2, d2=5, h=1.5, center=true, $fn=50);
        translate([0,0,25]) cylinder(d=6, h=50, center=true, $fn=50);
